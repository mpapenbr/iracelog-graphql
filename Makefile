...
.PHONY: install
## `install`: Fetch dependencies needed to run `iracelog-graphql`
install:
...
.PHONY: --test-runner
--test-runner:
	go test  `go list ./...` -p 1 -race -covermode=atomic -coverprofile=./coverage/coverage.txt
