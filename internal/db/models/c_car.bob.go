...
var CCarErrors = &cCarErrors{
	ErrUniqueCCarPkey: &UniqueConstraintError{s: "c_car_pkey"},
}
...
func (s CCarSetter) Overwrite(t *CCar) {
	...
	if !s.NameShort.IsUnset() {
		t.NameShort, _ = s.NameShort.Get()
	}
	if !s.CarID.IsUnset() {
		t.CarID, _ = s.CarID.Get()
	}
	if !s.CCarClassID.IsUnset() {
		t.CCarClassID, _ = s.CCarClassID.Get()
	}
	if !s.FuelPCT.IsUnset() {
		t.FuelPCT, _ = s.FuelPCT.Get()
	}
	if !s.PowerAdjust.IsUnset() {
		t.PowerAdjust, _ = s.PowerAdjust.Get()
	}
	if !s.WeightPenalty.IsUnset() {
		t.WeightPenalty, _ = s.WeightPenalty.Get()
	}
	if !s.DryTireSets.IsUnset() {
		t.DryTireSets, _ = s.DryTireSets.Get()
	}
}
...
func (s *CCarSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
		...
		if s.CCarClassID.IsUnset() {
			...
			vals[5] = psql.Arg(s.CCarClassID)
		}

		if s.FuelPCT.IsUnset() {
			vals[6] = psql.Raw("DEFAULT")
		} else {
			vals[6] = psql.Arg(s.FuelPCT)
		}

		if s.PowerAdjust.IsUnset() {
			vals[7] = psql.Raw("DEFAULT")
		} else {
			vals[7] = psql.Arg(s.PowerAdjust)
		}

		if s.WeightPenalty.IsUnset() {
			vals[8] = psql.Raw("DEFAULT")
		} else {
			vals[8] = psql.Arg(s.WeightPenalty)
		}

		if s.DryTireSets.IsUnset() {
			vals[9] = psql.Raw("DEFAULT")
		} else {
			vals[9] = psql.Arg(s.DryTireSets)
		}
...
func buildCCarJoins[Q dialect.Joinable](cols cCarColumns, typ string) cCarJoins[Q] {
	return cCarJoins[Q]{
		typ:         typ,
		Event:       cCarsJoinEvent[Q](cols, typ),
		CCarEntries: cCarsJoinCCarEntries[Q](cols, typ),
	}
}

func cCarsJoinEvent[Q dialect.Joinable](from cCarColumns, typ string) func(context.Context) modAs[Q, eventColumns] {
...
func cCarsJoinCCarEntries[Q dialect.Joinable](from cCarColumns, typ string) func(context.Context) modAs[Q, cCarEntryColumns] {
...
func (os CCarSlice) CCarEntries(mods ...bob.Mod[*dialect.SelectQuery]) CCarEntriesQuery {
	...
	return CCarEntries.Query(append(mods,
		sm.Where(psql.Group(CCarEntryColumns.CCarID).In(PKArgs...)),
	)...)
}
...
func PreloadCCarEvent(opts ...psql.PreloadOption) psql.Preloader {
...
func ThenLoadCCarEvent(queryMods ...bob.Mod[*dialect.SelectQuery]) psql.Loader {
	return psql.Loader(func(ctx context.Context, exec bob.Executor, retrieved any) error {
		loader, isLoader := retrieved.(interface {
			LoadCCarEvent(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
		})
		if !isLoader {
			return fmt.Errorf("object %T cannot load CCarEvent", retrieved)
		}

		err := loader.LoadCCarEvent(ctx, exec, queryMods...)
...
// LoadCCarEvent loads the cCar's Event into the .R struct
func (o *CCar) LoadCCarEvent(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}
...
// LoadCCarEvent loads the cCar's Event into the .R struct
func (os CCarSlice) LoadCCarEvent(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}
...
func ThenLoadCCarCCarEntries(queryMods ...bob.Mod[*dialect.SelectQuery]) psql.Loader {
	return psql.Loader(func(ctx context.Context, exec bob.Executor, retrieved any) error {
		loader, isLoader := retrieved.(interface {
			LoadCCarCCarEntries(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
		})
		if !isLoader {
			return fmt.Errorf("object %T cannot load CCarCCarEntries", retrieved)
		}

		err := loader.LoadCCarCCarEntries(ctx, exec, queryMods...)
...
// LoadCCarCCarEntries loads the cCar's CCarEntries into the .R struct
func (o *CCar) LoadCCarCCarEntries(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}
...
// LoadCCarCCarEntries loads the cCar's CCarEntries into the .R struct
func (os CCarSlice) LoadCCarCCarEntries(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}
