...
var CCarTeamErrors = &cCarTeamErrors{
	ErrUniqueCCarTeamPkey: &UniqueConstraintError{s: "c_car_team_pkey"},
}
...
func buildCCarTeamJoins[Q dialect.Joinable](cols cCarTeamColumns, typ string) cCarTeamJoins[Q] {
	return cCarTeamJoins[Q]{
		typ:       typ,
		CCarEntry: cCarTeamsJoinCCarEntry[Q](cols, typ),
	}
}

func cCarTeamsJoinCCarEntry[Q dialect.Joinable](from cCarTeamColumns, typ string) func(context.Context) modAs[Q, cCarEntryColumns] {
...
func PreloadCCarTeamCCarEntry(opts ...psql.PreloadOption) psql.Preloader {
...
func ThenLoadCCarTeamCCarEntry(queryMods ...bob.Mod[*dialect.SelectQuery]) psql.Loader {
	return psql.Loader(func(ctx context.Context, exec bob.Executor, retrieved any) error {
		loader, isLoader := retrieved.(interface {
			LoadCCarTeamCCarEntry(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
		})
		if !isLoader {
			return fmt.Errorf("object %T cannot load CCarTeamCCarEntry", retrieved)
		}

		err := loader.LoadCCarTeamCCarEntry(ctx, exec, queryMods...)
...
// LoadCCarTeamCCarEntry loads the cCarTeam's CCarEntry into the .R struct
func (o *CCarTeam) LoadCCarTeamCCarEntry(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}
...
// LoadCCarTeamCCarEntry loads the cCarTeam's CCarEntry into the .R struct
func (os CCarTeamSlice) LoadCCarTeamCCarEntry(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}
