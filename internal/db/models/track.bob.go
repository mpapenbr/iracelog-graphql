...
var TrackErrors = &trackErrors{
	ErrUniqueTrackPkey: &UniqueConstraintError{s: "track_pkey"},
}
...
func (s TrackSetter) Overwrite(t *Track) {
	...
	if !s.ShortName.IsUnset() {
		t.ShortName, _ = s.ShortName.Get()
	}
	if !s.Config.IsUnset() {
		t.Config, _ = s.Config.Get()
	}
	if !s.TrackLength.IsUnset() {
		t.TrackLength, _ = s.TrackLength.Get()
	}
	if !s.Sectors.IsUnset() {
		t.Sectors, _ = s.Sectors.Get()
	}
	if !s.PitSpeed.IsUnset() {
		t.PitSpeed, _ = s.PitSpeed.Get()
	}
	if !s.PitEntry.IsUnset() {
		t.PitEntry, _ = s.PitEntry.Get()
	}
	if !s.PitExit.IsUnset() {
		t.PitExit, _ = s.PitExit.Get()
	}
	if !s.PitLaneLength.IsUnset() {
		t.PitLaneLength, _ = s.PitLaneLength.Get()
	}
}
...
func (s *TrackSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
		...
		if s.Config.IsUnset() {
			...
			vals[3] = psql.Arg(s.Config)
		}

		if s.TrackLength.IsUnset() {
			vals[4] = psql.Raw("DEFAULT")
		} else {
			vals[4] = psql.Arg(s.TrackLength)
		}

		if s.Sectors.IsUnset() {
			vals[5] = psql.Raw("DEFAULT")
		} else {
			vals[5] = psql.Arg(s.Sectors)
		}

		if s.PitSpeed.IsUnset() {
			vals[6] = psql.Raw("DEFAULT")
		} else {
			vals[6] = psql.Arg(s.PitSpeed)
		}

		if s.PitEntry.IsUnset() {
			vals[7] = psql.Raw("DEFAULT")
		} else {
			vals[7] = psql.Arg(s.PitEntry)
		}

		if s.PitExit.IsUnset() {
			vals[8] = psql.Raw("DEFAULT")
		} else {
			vals[8] = psql.Arg(s.PitExit)
		}

		if s.PitLaneLength.IsUnset() {
			vals[9] = psql.Raw("DEFAULT")
		} else {
			vals[9] = psql.Arg(s.PitLaneLength)
		}
...
func buildTrackJoins[Q dialect.Joinable](cols trackColumns, typ string) trackJoins[Q] {
	return trackJoins[Q]{
		typ:    typ,
		Events: tracksJoinEvents[Q](cols, typ),
	}
}

func tracksJoinEvents[Q dialect.Joinable](from trackColumns, typ string) func(context.Context) modAs[Q, eventColumns] {
...
func (os TrackSlice) Events(mods ...bob.Mod[*dialect.SelectQuery]) EventsQuery {
	...
	return Events.Query(append(mods,
		sm.Where(psql.Group(EventColumns.TrackID).In(PKArgs...)),
	)...)
}
...
func ThenLoadTrackEvents(queryMods ...bob.Mod[*dialect.SelectQuery]) psql.Loader {
	return psql.Loader(func(ctx context.Context, exec bob.Executor, retrieved any) error {
		loader, isLoader := retrieved.(interface {
			LoadTrackEvents(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
		})
		if !isLoader {
			return fmt.Errorf("object %T cannot load TrackEvents", retrieved)
		}

		err := loader.LoadTrackEvents(ctx, exec, queryMods...)
...
// LoadTrackEvents loads the track's Events into the .R struct
func (o *Track) LoadTrackEvents(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}
...
// LoadTrackEvents loads the track's Events into the .R struct
func (os TrackSlice) LoadTrackEvents(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}
