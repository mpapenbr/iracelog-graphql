...
type Event struct {
	...
	IrSessionID       sql.Null[int32]  `db:"ir_session_id" `
...
func (s EventSetter) SetColumns() []string {
	...
	if s.NumCarTypes != nil {
		vals = append(vals, "num_car_types")
	}

	if s.NumCarClasses != nil {
		vals = append(vals, "num_car_classes")
	}

	if s.IrSessionID != nil {
	...
	if s.ReplayMinTimestamp != nil {
		vals = append(vals, "replay_min_timestamp")
	}

	if s.ReplayMinSessionTime != nil {
		vals = append(vals, "replay_min_session_time")
	}

	if s.ReplayMaxSessionTime != nil {
...
func (s EventSetter) Overwrite(t *Event) {
	...
	if s.RaceloggerVersion != nil {
		t.RaceloggerVersion = *s.RaceloggerVersion
	}
	if s.TeamRacing != nil {
		t.TeamRacing = *s.TeamRacing
	}
	if s.MultiClass != nil {
		t.MultiClass = *s.MultiClass
	}
	if s.NumCarTypes != nil {
		t.NumCarTypes = *s.NumCarTypes
	}
	if s.NumCarClasses != nil {
		t.NumCarClasses = *s.NumCarClasses
	}
	if s.IrSessionID != nil {
		t.IrSessionID = *s.IrSessionID
	}
	if s.TrackID != nil {
		t.TrackID = *s.TrackID
	}
	if s.PitSpeed != nil {
		t.PitSpeed = *s.PitSpeed
	}
	if s.ReplayMinTimestamp != nil {
		t.ReplayMinTimestamp = *s.ReplayMinTimestamp
	}
	if s.ReplayMinSessionTime != nil {
		t.ReplayMinSessionTime = *s.ReplayMinSessionTime
	}
	if s.ReplayMaxSessionTime != nil {
		t.ReplayMaxSessionTime = *s.ReplayMaxSessionTime
	}
	if s.Sessions != nil {
		t.Sessions = *s.Sessions
	}
	if s.IrSubSessionID != nil {
		t.IrSubSessionID = *s.IrSubSessionID
	}
	if s.TenantID != nil {
...
func (os EventSlice) Tenant(mods ...bob.Mod[*dialect.SelectQuery]) TenantsQuery {
	pkTenantID := make(pgtypes.Array[int32], len(os))
	for i, o := range os {
		pkTenantID[i] = o.TenantID
	}
	PKArgExpr := psql.Select(sm.Columns(
...
func (os EventSlice) Track(mods ...bob.Mod[*dialect.SelectQuery]) TracksQuery {
	pkTrackID := make(pgtypes.Array[int32], len(os))
	for i, o := range os {
		pkTrackID[i] = o.TrackID
	}
	PKArgExpr := psql.Select(sm.Columns(
...
func insertEventCCarEntries0(ctx context.Context, exec bob.Executor, cCarEntries1 []*CCarEntrySetter, event0 *Event) (CCarEntrySlice, error) {
	for i := range cCarEntries1 {
		cCarEntries1[i].EventID = &event0.ID
	}
