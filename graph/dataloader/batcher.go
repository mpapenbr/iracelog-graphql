...
/*
this is the overall contract for this batcher: the collector has to provide a map which keys are built by Key.String()
*/
...
// keys provide a String() func that provides a unique value over all keys used in that batch
func (t *genericMapBatcher[V]) get(ctx context.Context, keys dataloader.Keys) []*dataloader.Result {

	log.Printf("dataloader.genericMapBatcher.get, keys: [%s]\n", strings.Join(keys.Keys(), ","))
