...
func TestExtractEventSearchKeys(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "team (ci+trim)", args: args{arg: "Team: hallo "}, want: &events.EventSearchKeys{Team: "hallo"}},
		{name: "car", args: args{arg: "cAr: hallo "}, want: &events.EventSearchKeys{Car: "hallo"}},
		{name: "driver", args: args{arg: "driver:hallo "}, want: &events.EventSearchKeys{Driver: "hallo"}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("ExtractEventSearchKeys() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
